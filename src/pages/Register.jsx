...
import { FaHospital, FaUser, FaEnvelope, FaLock, FaPhone, FaEye, FaEyeSlash, FaStethoscope } from "react-icons/fa";
...
const Register = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    confirmPassword: "",
    role: "patient",
    specialty: null, // doctor জন্য
  });

  useEffect(() => {
    // Load specialties when doctor role is selected
    if (formData.role === "doctor") {
      console.log("Fetching specialties...");
      API.get("/specialties/")
        .then((res) => {
          console.log("Specialties loaded:", res.data);
          setSpecialties(res.data);
        })
        .catch((err) => {
          console.error("Failed to load specialties:", err);
          setError("Failed to load specialties. Please ensure backend is running.");
        });
    } else {
      setSpecialties([]);
    }
  ...
  const handleRoleChange = (role) => {
    ...
    setError("");
    setFormData({ ...formData, role, specialty: null });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Add specialty for doctors - MUST be integer
      if (formData.role === "doctor") {
        userData.specialty = Number(formData.specialty);
      }

      console.log("Sending registration data:", userData);
      console.log("Specialty type:", typeof userData.specialty, "Value:", userData.specialty);
      ...
      if (err.specialty || err.specialty_id) {
        errorMessage = Array.isArray(err.specialty) ? err.specialty[0] : (err.specialty || err.specialty_id);
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <select
                    ...
                    className="w-full pl-12 pr-4 py-3 border-2 border-gray-200 rounded-xl focus:border-purple-500 focus:outline-none transition appearance-none"
                  >
                ...
                {specialties.length === 0 && (
                  <p className="text-sm text-amber-600 mt-1">
                    ⚠️ Loading specialties... Make sure backend is running.
                  </p>
                )}
              </div>
            )}

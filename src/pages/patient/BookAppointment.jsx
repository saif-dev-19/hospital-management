...
const BookAppointment = () => {
  ...
  const [appointmentTypes] = useState([
    "Consultation", "Check-up", "Emergency"
  ]);
  ...
  useEffect(() => {
    ...
    const specialty = specialties.find(s => s.name === formData.specialty);
    if (!specialty) return;
    console.log("s",specialty);
    
    Promise.all(specialty.doctors.map(id => API.get(`/doctors/${id}/`)))
      .then(results =>setDoctors(results.map(r => r.data)))
      .catch(err => console.error(err));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const body = {
        doctor_id: doctorId,
        // Assuming you have the patient ID stored or from JWT
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8 text-center">
          ...
          <div className="bg-gradient-to-r from-rose-50 to-pink-50 rounded-xl p-6 mb-6">
            <div className="space-y-2">
              <div className="flex justify-between"><span className="text-gray-600">Doctor:</span><span className="font-semibold text-gray-800">{formData.doctor.full_name}</span></div>
              <div className="flex justify-between"><span className="text-gray-600">Date:</span><span className="font-semibold text-gray-800">{formData.date}</span></div>
              <div className="flex justify-between"><span className="text-gray-600">Time:</span><span className="font-semibold text-gray-800">{formData.time}</span></div>
              <div className="flex justify-between"><span className="text-gray-600">Type:</span><span className="font-semibold text-gray-800">{formData.type}</span></div>
            </div>
          </div>
          <button
            onClick={() => { setStep(1); setFormData({specialty:"", doctor:"", date:"", time:"", type:""}) }}
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-lg p-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                {doctors.map(doctor => (
                  ...
                  >
                    ...
                    <div className="font-bold text-lg">{doctor.profile?.full_name}</div>
                    <div className="text-gray-600">{doctor.specialty.name}</div>
                  </button>
                ))}
              </div>
              <div className="flex space-x-4">
                <button type="button" onClick={() => setStep(1)} className="flex-1 py-3 border-2 border-gray-200 rounded-xl font-semibold hover:bg-gray-50 transition">Back</button>
                <button type="button" onClick={() => formData.doctor && setStep(3)} disabled={!formData.doctor} className="flex-1 py-3 bg-gradient-to-r from-rose-600 to-pink-600 text-white rounded-xl font-semibold shadow-lg hover:shadow-xl transition disabled:opacity-50">Next Step</button>
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div>
              ...
              <div className="flex space-x-4">
                <button type="button" onClick={() => setStep(2)} className="flex-1 py-3 border-2 border-gray-200 rounded-xl font-semibold hover:bg-gray-50 transition">Back</button>
                <button type="submit" disabled={!formData.date || !formData.time} className="flex-1 py-3 bg-gradient-to-r from-rose-600 to-pink-600 text-white rounded-xl font-semibold shadow-lg hover:shadow-xl transition disabled:opacity-50">Confirm Booking</button>
              </div>
            </div>
          )}
        </form>
